...
class Database:
    def __init__(self):
        ...
        self.channel_members = self.db['channel_members']
    ...
    async def create_indexes(self):
        ...
        try:
            ...
            await self.channel_members.create_index([("user_id", 1), ("channel_id", 1)], unique=True)
            await self.channel_members.create_index("user_id")
            await self.channel_members.create_index("status")
            await self.banned_users.create_index("user_id", unique=True)
            await self.banned_users.create_index("active")
            await self.recent_updates.create_index([("timestamp", -1)])
