...
async def search_drama(query: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for doc in all_shows:
            ...
            show_slug = normalize_slug(show_name)  # normalized for comparison
            category = doc.get("category", "").lower()
            key = f"{show_slug}_{category}"
            
            q_slug = normalize_slug(q)
            if show_slug == q_slug:
                scored_results[key] = (doc, 100)
            elif q_slug in show_slug.split():
                scored_results[key] = (doc, 90)
            elif category == q_slug:
                scored_results[key] = (doc, 85)
        
        # Priority 2: PARTIAL/SUBSTRING matches
        if len(scored_results) < limit:
            for doc in all_shows:
                show_slug = normalize_slug(doc.get("show_name", ""))
                category = doc.get("category", "").lower()
                key = f"{show_slug}_{category}"
                q_slug = normalize_slug(q)

                if key not in scored_results:
                    if q_slug in show_slug and len(q_slug) >= 2:
                        scored_results[key] = (doc, 80)
                    elif show_slug.startswith(q_slug) and len(q_slug) >= 2:
